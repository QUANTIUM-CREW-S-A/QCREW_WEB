...
export function Problem() {
  ...
  return (
    <SectionWrapper className="bg-rack-paper relative overflow-hidden py-24">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-16 items-stretch relative">
          ...
          <div className="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 bg-rack-paper border border-rack-rule rounded-full p-2 text-rack-ink/50 shadow-xl">
          ...
          >
            <Card className="h-full bg-red-950/5 border-red-500/10 hover:border-red-500/20 transition-all p-8 relative overflow-hidden group">
              ...
              <h3 className="text-2xl font-bold text-rack-ink mb-2 flex items-center gap-3">
              ...
              <div className="space-y-6">
                {problems.map((item, idx) => (
                  <div key={idx} className="flex gap-4 items-start">
                    ...
                    <p className="text-rack-graph group-hover:text-red-100/80 transition-colors">{item}</p>
                  </div>
                ))}
              </div>
            </Card>
          ...
          >
            <Card className="h-full bg-brand-primary/5 border-brand-primary/20 hover:border-brand-primary/40 transition-all p-8 relative overflow-hidden group shadow-[0_0_50px_rgba(0,212,255,0.05)]">
              ...
              <div className="relative z-10">
                <h3 className="text-2xl font-bold text-rack-ink mb-2 flex items-center gap-3">
                  <span className="w-8 h-8 rounded-full bg-brand-primary/10 flex items-center justify-center border border-brand-primary/20 text-rack-brand text-sm">2</span>
                  La Experiencia Quantium
                </h3>
                <p className="text-rack-brand/60 mb-8 text-sm">Soluciones diseñadas para escalar sin límites</p>
                ...
                <div className="space-y-6">
                  {solutions.map((item, idx) => (
                    ...
                    >
                      <div className="mt-1 min-w-[20px]">
                        <CheckCircle2 className="w-5 h-5 text-rack-brand drop-shadow-[0_0_8px_rgba(0,212,255,0.5)]" />
                      </div>
                      <p className="text-rack-ink group-hover:text-rack-ink transition-colors font-medium">{item}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
